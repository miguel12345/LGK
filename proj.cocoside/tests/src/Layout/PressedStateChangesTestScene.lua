...
function TestScene.create()
    ...
    scene.execute = function(self)
        ...
        -- Check for multi touch problems, If the user presses with a second finger while the element is already pressed, the background color should remain the same
        cc.Director:getInstance():getOpenGLView():simulateSecondFingerPressDown(x,y)
        elementBackgroundColor = element:getBackGroundColor()
        textColor = self.elements["text"]:getVirtualRenderer():getTextColor()
        testassert(elementBackgroundColor.r==0 and elementBackgroundColor.g==255 and elementBackgroundColor.b==0,"The element with pressed backgroundColor set to green must be green when two fingers are pressing down on it")
        testassert(textColor.r==255 and textColor.g==0 and textColor.b==0,"The text with pressed textColor set to red must be red when two fingers are pressing down on it")
        ...
        -- Check for multi touch problems, If the user releaseses the second finger the background color should stay like it was
        cc.Director:getInstance():getOpenGLView():simulateSecondFingerPressUp()
        elementBackgroundColor = element:getBackGroundColor()
        textColor = self.elements["text"]:getVirtualRenderer():getTextColor()
        testassert(elementBackgroundColor.r==255 and elementBackgroundColor.g==0 and elementBackgroundColor.b==0,"The element with backgroundColor set to red must be red when NOT pressing down on it")
        testassert(textColor.r==0 and textColor.g==255 and textColor.b==0,"The text with textColor set to green must be green when pressing down on it")
